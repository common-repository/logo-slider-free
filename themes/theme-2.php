<?php
    ...
    $content .='<style>';
	    $content .='
	    	#lsuw-'.$postid.' .lsuw-logo-gird-2-thumbnail{
				margin:'.$lsuw_margin_size.'px;
				border:1px solid #'.$lsuw_border_color.';
	    	}
	    	#lsuw-'.$postid.' .lsuw-logo-gird-2-thumbnail:hover{
		...
		$content .='
			...
			';	    	
		$content .='
			.lsuw-logo-gird-2 img {
			  height: '.$lsuw_logo_height.'px;
			  width: 100%;
			  box-shadow:none;
			  border-radius:0;
			}';
		$content .='.lsuw-logo-gird-2:after{
			  background:'.$lsuw_title_overlaycolor.';	
			}';	
		$content.='
			.lsuw-logo-gird-2 .lsuw-logo-gird-2-content h3.lsuw-logo-gird-2-title{
	...
	if($lsuw_hide_tooltips == 1){
		$content.='
			<div class="lsuw-logo-gird-2-thumbnail">
				<div class="lsuw-logo-gird-2 img-tipso-'.$postid.' tipso_style" data-tipso="'.esc_attr(get_the_title()).'">';
					if($lsuw_hide_logo == 1){
						$content.='<img src="'.$post_thumb.'" alt="'.esc_attr(get_the_title()).'" />';
					};
					if($lsuw_hide_links == 1){
						$content.='
						<ul class="lsuw-logo-gird-2-icon">
							<li><a target="'.$open_url.'" href="'.esc_url($web_url).'"><i class="fa fa-link"></i></a></li>
						</ul>';
					};
					if($lsuw_hide_title == 1){
						$content.='
						<div class="lsuw-logo-gird-2-content">
							<h3 class="lsuw-logo-gird-2-title">'.esc_attr(get_the_title()).'</h3>
						</div>';			
					};
					$content.='
				</div>';
				if($lsuw_hide_desc == 1){
					$content.='<div class="lsuw-logo-gird-2-description">'.lauw_get_excerpt(86).'</div>';
				};
				$content.='
			</div>';				
		}
		...
		$content.='
			...
			<div class="lsuw-logo-gird-2">';
				...
				if($lsuw_hide_desc == 1){
					$content.='<div class="lsuw-logo-gird-2-description">'.lauw_get_excerpt($lsuw_excerpt).'</div>';
				};
				$content.='
			</div>';					
		}
