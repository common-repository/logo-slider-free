<?php
	...
	$content .='
		#lsuw-'.$postid.' .lsuw_slider_style1{
			border: 1px solid #'.$lsuw_border_color.';
		}';
	$content .='
		#lsuw-'.$postid.' .lsuw_slider_style1:hover{
	...
	if($lsuw_hide_tooltips == 1){
		...
		<div class="item-'.$postid.'">
			<div class="lsuw_slider_style1 img-tipso-'.$postid.' tipso_style" data-tipso="'.esc_attr(get_the_title()).'">';
				if($lsuw_hide_logo == 1){
					$content.='<div class="lsuw_slider_style1-link">';

					if($lsuw_hide_links == 1){
						$content.='<a target="'.$open_url.'" href="'.esc_url($web_url).'"><img src="'.$post_thumb.'" alt="'.esc_attr(get_the_title()).'" /></a>';
					}
					else{
				...
				if($lsuw_hide_title == 1){
					$content.='
						<div class="lsuw_slider_style1-title">';
							$content.=''.get_the_title().'
						</div>
					';
				}
				if($lsuw_hide_desc == 1){
					$content.='<div class="lsuw-logo-gird-1-description">'.lauw_get_excerpt($lsuw_excerpt).'</div>';
				};
				$content.='
			</div>
		</div>';
